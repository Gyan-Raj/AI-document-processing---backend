...
async def run_assessment_and_generate_summary_route(
    request: Request,
    background_tasks: BackgroundTasks,
    data: RunAssessmentAndGenerateSummaryRequest,
):
    summary = await run_assessment_and_generate_summary(
        request.state.user_id, data.project_id, background_tasks
    )
    return {
